...
public class SDKParser extends SDKScanner {
	...
	public SDKParser(SyntaxTree parseTree) {
	...
	public boolean program(SyntaxTree sT) {
		byte[] funSet = {FUN};
		boolean value = true;
	...
	boolean statement(SyntaxTree sT) {
		...
		if (ifStatement(sT))
			return statement(sT);
		else if (whileStatement(sT))
			return statement(sT);
		else if (expression(sub.insertSubtree(EXPRESSION)) && semicolon(sub))
			return statement(sT);
		return false;
	...
	private boolean function(SyntaxTree subtree) {
		...
		if (match(true, subtree, openParSet)) {
			...
			if (match(true, subtree, closeParSet))
				return fun && funOpen(subtree) && statement(subtree)
						&& funClose(subtree);
			else {// Syntaxfehler
	...
	private boolean expressionList(SyntaxTree subtree) {
		...
		if (lookAhead(kommaSet))
			return expression(subtree.insertSubtree(EXPRESSION))
					&& match(true, subtree, kommaSet)
	...
	boolean operator(SyntaxTree sT) {
		...
		else if (lookCurrent(numSet))
			// operator -> num
			return number(sT.insertSubtree(NUM));

		else if (lookAhead(BOOL_OPERATOR)) {
			SyntaxTree sub = sT.insertSubtree(BOOL_STATEMENT);
			return match(true, sub, identSet) && match(true, sub, BOOL_OPERATOR)
					&& expression(sub);

		} else if (lookAhead(EQUAL)) {
			SyntaxTree sub = sT.insertSubtree(ASSERTION);
			return match(true, sub, identSet) && match(true, sub, EQUAL)
					&& expression(sub);

		} else if (lookAhead(openParSet)
				&& functionCall(sT.insertSubtree(FUNCTION_CALL)))
			return true;
		...
		// wenn das nicht möglich ...
			...
			syntaxError(
					"Ziffer, Identifier, Klammer auf oder Function Call erwartet");
			return false;
		}
	}// operator

	private boolean number(SyntaxTree sT) {
		boolean value = match(true, sT, DIGIT);
		if (lookCurrent(DIGIT))
			return value && number(sT.insertSubtree(NUM));
		else
			return value;
	}

	private boolean digit(SyntaxTree syntaxTree) {
		byte[] numSet = {DIGIT};
		return match(true, syntaxTree, numSet);
	}

	private boolean whileStatement(SyntaxTree sT) {
		if (lookCurrent(WHILE)) {
			SyntaxTree subTreeIf = sT.insertSubtree(WHILE_STATEMENT);
			return match(true, subTreeIf, WHILE)
					&& match(true, subTreeIf, OPEN_PAR)
					&& expression(subTreeIf.insertSubtree(EXPRESSION))
					&& match(true, subTreeIf, CLOSE_PAR) && funOpen(subTreeIf)
					&& statement(subTreeIf) && funClose(subTreeIf);
		}
		return false;
	}

	private boolean ifStatement(SyntaxTree sT) {
		if (lookCurrent(IF)) {
			SyntaxTree subTreeIf = sT.insertSubtree(IF_STATEMENT);
			return match(true, subTreeIf, IF)
					&& match(true, subTreeIf, OPEN_PAR)
					&& expression(subTreeIf.insertSubtree(EXPRESSION))
					&& match(true, subTreeIf, CLOSE_PAR) && funOpen(subTreeIf)
					&& statement(subTreeIf) && funClose(subTreeIf);
	...
	boolean match(boolean addLexem, SyntaxTree sT, byte... matchSet) {
		...
		for (byte b : matchSet)
			if (tokenStream.get(pointer).token.getToken() == b) {
				...
				SyntaxTree sub = sT.insertSubtree(
						tokenStream.get(pointer).token.getToken());
				if (addLexem)
	...
	void syntaxError(String s) {
		...
		if (tokenStream.get(pointer).token.getToken() == EOF)
			System.out.println("Syntax Fehler in Zeile "
					+ tokenStream.get(pointer).line + ": " + "EOF");
		else
			System.out.println(
					"Syntax Fehler in Zeile " + tokenStream.get(pointer).line
							+ ": " + tokenStream.get(pointer).token);
		System.out.println(s);
	}// syntaxError
